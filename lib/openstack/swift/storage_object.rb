  ...
  class StorageObject
    ...
    def self.create(container, objectname, headers={}, data=nil)
      ...
      if data.nil? #just create an empty object
        ...
        container.swift.connection.req("PUT", CGI.escape(path), {:headers=>provided_headers})
      else
    ...
    def object_metadata
      ...
      response = @container.swift.connection.req("HEAD", CGI.escape(path))
    ...
    def data(size = -1, offset = 0, headers = {})
      ...
      path = "/#{@containername}/#{@name}"
      begin
        response = @container.swift.connection.req("GET", CGI.escape(path), {:headers=>headers})
        response.body
    ...
    def data_stream(size = -1, offset = 0, &block)
      ...
      path = @container.swift.connection.service_path + CGI.escape("/#{@containername}/#{@name}")
    ...
    def set_metadata(metadatahash)
      ...
      path = "/#{@containername}/#{@name}"
      begin
        response = @container.swift.connection.req("POST", CGI.escape(path), {:headers=>headers})
      rescue OpenStack::Exception::ItemNotFound => not_found
    ...
    def copy(object_name, container_name, headers = {})
      ...
      begin
        response = @container.swift.connection.req("PUT", CGI.escape(path), {:headers=>provided_headers})
      rescue OpenStack::Exception::ItemNotFound => not_found
